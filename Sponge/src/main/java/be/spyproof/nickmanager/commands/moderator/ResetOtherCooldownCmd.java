 ...
 */
public class ResetOtherCooldownCmd extends AbstractCmd
{
    private static final String ARG = "player";

    private ResetOtherCooldownCmd(MessageController messageController, ISpongePlayerController playerController)
    {
    ...
    public CommandResult execute(CommandSource src, CommandContext args) throws CommandException
    {
        ...
        if (player.isPresent())
        {
            player.get().sendMessage(this.messageController.getMessage(Reference.SuccessMessages.ADMIN_NICK_RESET_COOLDOWN_RECEIVED).toText());
        }

        src.sendMessage(this.messageController.getMessage(Reference.SuccessMessages.ADMIN_NICK_RESET_COOLDOWN).apply(TemplateUtils.getParameters(playerData.get())).build());
        return CommandResult.success();
    }
