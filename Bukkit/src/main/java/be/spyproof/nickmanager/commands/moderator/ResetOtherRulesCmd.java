 ...
 */
public class ResetOtherRulesCmd extends AbstractCmd implements TabCompleter
{
    private static final String ARG = "player";

    public ResetOtherRulesCmd(MessageController messageController, IBukkitPlayerController playerController, String... keys)
    {
    ...
    {
        src.sendMessage(this.messageController.getFormattedMessage(Reference.HelpMessages.ADMIN_NICK_RESET_RULES));
    }
    ...
    public void execute(CommandSender src, String cmd, String[] args)
    {
        ...
        if (player != null)
        {
            player.sendMessage(this.messageController.getFormattedMessage(Reference.SuccessMessages.ADMIN_NICK_RESET_RULES_RECEIVED));
        }

        src.sendMessage(TemplateUtils.apply(this.messageController.getFormattedMessage(Reference.SuccessMessages.ADMIN_NICK_RESET_RULES), playerData.get()));
    }
